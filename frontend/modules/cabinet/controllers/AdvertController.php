...
class AdvertController extends AuthController
{
  ...
  public function actionFileUploadImages()
  {
    if (Yii::$app->request->isPost) {
        ...
        BaseFileHelper::createDirectory($path);
        $file = UploadedFile::getInstanceByName('images[0]');
  ...
  public function actionCreate()
  {
      ...
      if ($model->load(Yii::$app->request->post()) && $model->save()) {
          return $this->redirect(['step2']); // (['view', 'id' => $model->idadvert]);
      }
  ...
  public function actionStep2(){
    ...
    if($general_image = $model->general_image){
        $image[] =  '<img src="/uploads/adverts/' . $model->idadvert . '/general/small_' . $general_image . '" class="file-preview-image">';
    }
      ...
      if (is_dir($path)) {
        ...
        foreach ($files as $file) {
          if (strstr($file, "small_") && !strstr($file, "general")) {
            $images_add[] = '<img src="/uploads/adverts/' . $model->idadvert . '/' . basename($file) . '" class="file-preview-image">';
          }
        }
      }
